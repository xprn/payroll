...
const debug        = require('debug')('payroll:router');
...
module.exports = function (root) {
    ...
    obj.getRoutes = () => routes.slice().map(cleanRouteData);
    obj.auth      = (controller, header) => {
        authController = controller;
        authHeader     = header;
        return obj;
    };
    obj.route     = (method, endpoint) => {
        let container = {
        ...
        let mod = {
            ...
            auth:        (flag) => {
                container.authenticate = async (req, res, next) => {
                    ...
                    authController.isTokenAuthorized(token, flag)
                        .then(() => next())
                        .catch(err => {
                            switch (err.name) {
            ...
            validation:  descriptor => {
                container.validate = (req, res, next) => {
                    ...
                    // Query validation
                    if (!(descriptor && descriptor._skipQueryValidation)) Object.entries(container.data.query)
                    ...
                    // Header validation
                    if (!(descriptor && descriptor._skipHeadersValidation)) Object.entries(container.data.headers)
                    ...
                    // Body validation
                    if (!(descriptor && descriptor._skipBodyValidation)) Object.entries(container.data.body)
